package org.nzy.sso.service.impl;
...
import org.nzy.sso.dao.JedisClient;
...
public class UserLoginServiceImpl implements UserLoginService {
	...
	//注册
	@Override
	public FjnyResult doReg(String username, String password, String phone, String email) {
		TbUser record = new TbUser();
		record.setUsername(username);
		String md5Pwd = DigestUtils.md5DigestAsHex(password.getBytes());
		record.setPassword(md5Pwd);
		record.setPhone(phone);
		record.setEmail(email);
		record.setUpdated(new Date());
		record.setCreated(new Date());
		int selective = tbUserMapper.insertSelective(record);
		if(selective > 0) {
