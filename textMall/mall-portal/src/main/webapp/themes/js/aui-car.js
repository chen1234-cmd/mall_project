    ...
    $(".aui-car-box").each(function() { //循环每个店铺
      ...
      $(this).find(".goodsCheck").each(function() { //循环店铺里面的商品
        if ($(this).is(":checked")) { //如果该商品被选中
            var num = parseInt($(this).parents(".shop-info").find(".num").html()); //得到商品的数量
			alert(num);
            var price = parseFloat($(this).parents(".shop-info").find(".price").html()); //得到商品的单价
            var total = price * num; //计算单个商品的总价
          oprice += total; //计算该店铺的总价
        }
        $(this).closest(".aui-car-box").find(".ShopTotal").html(oprice.toFixed(2)); //显示被选中商品的店铺总价
      });
